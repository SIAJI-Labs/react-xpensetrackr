...
export default function BudgetDeleteDialog({ openState, setOpenState }: dialogProps){
    const [budgetUuid, setBudgetUuid] = useState<string>();
    useEffect(() => {
        // Listen to Budget Dialog event
        ...
        window.addEventListener('budget.delete-action', handleDeleteAction);
        // Remove the event listener when the component unmounts
        return () => {
            window.removeEventListener('budget.delete-action', handleDeleteAction);
        };
    }, []);
    ...
    const confirmDelete = ($refs: any) => {
        ...
        if(budgetUuid){
            ...
            // Make delete request
            axios.post(route('api.budget.v1.destroy', budgetUuid), formData, {
                ...
                if (response.status === 200) {
                    ...
                    setTimeout(() => {
                        if(setOpenState){
                            ...
                            document.dispatchEvent(new CustomEvent('budget.deleted-action', {
                                bubbles: true,
                                detail: {
                                    action: 'delete'
                                }
    ...
    return (
        <section id={ `budget-deleteDialogSection` }>
