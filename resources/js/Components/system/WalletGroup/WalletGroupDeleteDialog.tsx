...
export default function WalletGroupDeleteDialog({ openState, setOpenState }: dialogProps){
    ...
    useEffect(() => {
        ...
        window.addEventListener('wallet-group.delete-action', handleDeleteAction);
        // Remove the event listener when the component unmounts
        return () => {
            window.removeEventListener('wallet-group.delete-action', handleDeleteAction);
        };
    }, []);
    ...
    const confirmDelete = ($refs: any) => {
        ...
        if(walletUuid){
            ...
            // Make delete request
            axios.post(route('api.wallet-group.v1.destroy', walletUuid), formData, {
                ...
                if (response.status === 200) {
                    ...
                    setTimeout(() => {
                        if(setOpenState){
                            ...
                            document.dispatchEvent(new CustomEvent('wallet-group.deleted-action', {
                                bubbles: true,
                                detail: {
                                    action: 'delete'
                                }
    ...
    return (
        <section id={ `walletGroup-deleteDialogSection` }>
