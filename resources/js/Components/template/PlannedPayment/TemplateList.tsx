...
export default function TemplateList({ plannedPayment, deleteAction = true, editAction = true }: PropsWithChildren<TemplateProps> ){
            ...
            <div className={ ` flex flex-col gap-2 border rounded p-4 cursor-pointer` }>
                ...
                <div className={ ` flex flex-row gap-6 justify-between` }>
                    <span className={ ` font-medium w-full md:w-auto` }>
                        { moment((plannedPayment && 'date_start' in plannedPayment ? (plannedPayment?.date_start) : moment())).format('MMM Do, YYYY') }
                        </span>

                    <div className={ ` flex flex-row flex-1 md:flex-none justify-between gap-2 items-center` }>
                        <span className={ ` font-normal whitespace-nowrap ${plannedPayment && 'type' in plannedPayment ? (plannedPayment?.type === 'expense' ? ` text-red-500` : ( plannedPayment.type === 'income' ? `text-green-500` : ` dark:text-white`)) : ``}` }>
                            { formatRupiah((plannedPayment && 'amount' in plannedPayment ? (plannedPayment?.amount) : 0) + (plannedPayment && 'extra_amount' in plannedPayment ? (plannedPayment?.extra_amount) : 0)) }
                        </span>
                            ...
                            <DropdownMenu open={openDropdown} onOpenChange={setOpenDropdown}>
                                ...
                                <DropdownMenuContent sideOffset={5} alignOffset={0} side={ `left` } align={ `start` }>
                                    ...
                                    {(() => {
                                        ...
                                        let dialogSection = document.getElementById('plannedPayment-dialogSection');
                                        if(editAction && dialogSection){
                ...
                {(() => {
                    ...
                    // Tags
                    if(plannedPayment && 'planned_payment_tags' in plannedPayment && plannedPayment?.planned_payment_tags && plannedPayment.planned_payment_tags.length > 0){
                        information.push(
                            <Badge variant={ `secondary` } className={ ` rounded flex flex-row gap-1 items-center` } key={ `plannedPayment_tags-${plannedPayment?.uuid}` }>
