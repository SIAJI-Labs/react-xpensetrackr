import { PageProps, RecordItem, WalletShareItem, WalletItem } from "@/types"
...
export default function Show({ auth, data }: PageProps<ContentProps>) {
    ...
    const [shareItem, setShareItem] = useState<WalletItem[] | undefined>([]);

    useEffect(() => {
        setShareItem(data.wallet_share_item);
    }, [data]);
    ...
    useEffect(() => {
        const handleDialogEvent = (event: any) => {
            console.log(event);
            if(event.detail?.action && event.type === 'wallet-share.deleted-action' && event.detail?.action === 'delete'){
                router.visit(route('sys.wallet-share.index'));
            } else {
                router.reload({
                    only: ['data']
                });
    ...
    return (
        <>
            <SystemLayout
                user={auth.user}
                header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Wallet Share Detail: { `${data?.name}` }</h2>}
            >
                <Head title={ `Wallet Share: ${data?.name}` } />
                    ...
                    <Card className={ ` w-full` }>
                        <CardHeader>
                            <div className={ ` relative flex flex-row justify-between items-start gap-4` }>
                                <div>
                                    <CardTitle>
                                            <div>Wallet Share Detail: { `${data?.name}` }</div>
                                    </CardTitle>
                                    <CardDescription>See summary of <u>{ `${data?.name}` }</u> wallet share</CardDescription>
                                </div>
